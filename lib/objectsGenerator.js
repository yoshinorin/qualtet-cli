...
function objectsGenerator(content, contentType, url) {
  ...
  let path = content.path.endsWith("/") ? content.path : content.path.slice(0, content.path.lastIndexOf("/") + 1);
  if (!path.startsWith("/")) {
    path = "/" + path;
  }
  ...
  const c = content._content.replaceAll('{% raw %}','').replaceAll('{% endraw %}','');

  if (micromatch.isMatch(path, ['temp/*', 'all-categories/', 'all-archives/', 'scaffolds/', '404/', '_drafts/'])) {
    log.info(`skiped - : ${path}`);
    return;
  }

  const htmlContent = externalLink(md.render(codeBlockFormatter.format(c)), url);
  const data = {
