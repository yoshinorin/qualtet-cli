...
async function publish(item, options) {
  ...
  try {
    ...
    logInfo(
      `${contentType.charAt(0).toUpperCase() + contentType.slice(1)} published: ${data.id} - ${data.path}`,
    );
    return data;
  } catch (error) {
    logError(
      `${contentType.charAt(0).toUpperCase() + contentType.slice(1)} publish failed: ${payload.path} - ${error}`,
    );
    throw error;
  }
}
